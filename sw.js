...
;(function (global, factory) {
  ...
  var HappyScrollStrip = {
    render: function render() {
      ...
      return _c(
        ...
        {
          ref: 'stripContainer',
          staticClass: 'happy-scroll-strip',
          class: [
            _vm.horizontal
              ? 'happy-scroll-strip--horizontal'
              : 'happy-scroll-strip--vertical',
          ],
          style: [_vm.initLocation],
          on: {
            '!wheel': function wheel($event) {
              $event.stopPropagation()
              return _vm.handlerWheel($event)
            },
          },
        },
        [
          _c('div', {
            ...
            style: [
              _vm.translate,
              _defineProperty({}, _vm.config.sizeAttr, _vm.length + 'px'),
              _vm.initSize,
              { background: _vm.color },
              { opacity: _vm.isOpacity },
            ],
            on: {
              mousedown: function mousedown($event) {
                $event.stopPropagation()
                return _vm.handlerMouseDown($event)
              },
            },
          }),
        ]
    ...
    methods: {
      ...
      changeOffset: function changeOffset(offset, event) {
        ...
        this.currentOffset = offset
        // 偏移
        this.$refs.strip.style.transform =
          this.config.translate + '(' + offset + 'px)'
  ...
  var scroll = function (options) {
    ...
    function uninstall(element) {
      ...
      state.onAnimationStart &&
        removeEvent(state.container, 'animationstart', state.onAnimationStart)
  ...
  var elementResizeDetector = function (options) {
    ...
    var batchProcessor$$1 = getOption(
      options,
      'batchProcessor',
      batchProcessor({ reporter: reporter$$1 })
    )
  ...
  var HappyScroll = {
    render: function render() {
      ...
      return _c(
        'div',
        { ref: 'happy-scroll', staticClass: 'happy-scroll' },
        [
          _c(
            'div',
            {
            ...
            ? _c(
                ...
                _vm._g(
                  _vm._b(
                    {
                      ref: 'stripY',
                      attrs: { throttle: _vm.throttle, move: _vm.moveY },
                      on: { change: _vm.slideYChange },
                    },
                    'happy-scroll-strip',
                    _vm.$attrs,
                    false
    ...
    mounted: function mounted() {
      ...
      this.$nextTick(function () {
        ...
        _this2.$nextTick(function () {
          ...
          // 将视图dom移动到设定的位置
          _this2.scrollTop &&
            (_this2.$refs.container.scrollTop = +_this2.scrollTop)
          _this2.scrollLeft &&
            (_this2.$refs.container.scrollLeft = +_this2.scrollLeft)
        })
      })
